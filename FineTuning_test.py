...
if __name__ == '__main__':
    ...
    #out = DnCNN_test.eval(model_name=model_fineTuned_name, model_path='./fineTuning_weight/', show_imput=False)
    ...
    #utility.show_image(img_fineTun, title='watermark_fineTuningAttack_10epochs')

    img_logo_watermarked = watermarkingTest.eval(DnCNN_model_name=utility.get_last_model('./overwriting/'), model_path='./overwriting/')
    ...
    img_results = utility.create_text_image(utility.create_empty_image(img_logo_watermarked.shape[0], img_logo_watermarked.shape[1]), text)

    utility.show_image(utility.stack_images_square([img_logo_original, img_logo_watermarked, img_logo_fineTun, img_results]), '1 dncnn original ,2 Watermarked, 3 FineTuned, 4 WM result: {:.4f}<=0.00607'.format(dist))
