...
class ExecuteVerification:
    ...
    def verificationOnFineTunedImg(self, model_attacked_name='fineTuningDnCNNcman_weight_10'):
        ...
        opt_ver_img = DnCNN_test.eval(model_name=utility.get_last_model('./overwriting/'), model_path='./overwriting/',
                                      test_img='key_imgs/trigger_image.png', show_imput=False)
        new_ver_img = DnCNN_test.eval(model_name=model_attacked_name, model_path='./fineTuning_weight/',
                                      test_img='key_imgs/trigger_image.png', show_imput=False)
    ...
    def verificationMoreTriggerImgs(self, trigger, triggers_false, model_path='./overwriting/'):
        ...
        opt_ver_img = DnCNN_test.eval(model_name=utility.get_last_model(model_path), model_path=model_path,
                                      test_img=trigger, show_imput=False)
        result = []
        for img in triggers_false:
            new_ver_img = DnCNN_test.eval(model_name=utility.get_last_model(model_path), model_path=model_path,
                                          test_img=img, show_imput=False)
            dist, watermark_succeeded = self.watermark_verification(opt_ver_img, new_ver_img, dim_triggerImg)
